...
// 	        $.ajax({
//             type: "POST",
//             url : linke+"Return_penjualan/show_return_penjualan",
//             data :str,
  ...
  //  var nama_pos=$("#nama_pos").val(); 
    ...
    if(no_faktur!=""){
                    $("#result").html("<img style='display: block; margin: 0 auto; text-align: center; ' src='"+linke+"assets/img/ajax-loader.gif'/>");
                     $.ajax({
                        type:"post",
                        "url": linke+"Return_penjualan/create_return_id/"+no_faktur,
...
function actiondetail(value, row, index){
 ...
 function detailnya(nama_cus, nama_barang, jumlah_barang, satuan, harga_penjualan, no_faktur,id_detail_penjualan){
        console.log("id_detail_penjualan : " + id_detail_penjualan + ", no faktur: " + no_faktur);
        var str = nama_cus;
        var res = str.replace('|', '"');
        res = res.replace('|', '"');
        res = res.replace("]", "'");
        ...
        document.getElementById('no_faktur_detail').value = no_faktur;
        console.log(document.getElementById('no_faktur_detail').value);
