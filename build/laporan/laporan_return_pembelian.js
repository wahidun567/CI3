...
$("#filter_all").click(function () {
       var url=linke+"Laporan/get_return_pembelian_tampil_pakai/";
       var tgl_1=$("#tgl_1").val();
       var tgl_2=$("#tgl_2").val();
       if (tgl_1 != "" && tgl_2!="") {
         //console.log(url+"?tgl_1="+tgl_1+"&tgl_2="+tgl_2);
              $('#table').bootstrapTable('refresh', {url: linke+'Laporan/get_return_pembelian_tampil_pakai/' + tgl_1 + '/' + tgl_2});
              saldo_asli = 0;
       }else {
...
function jenis(value){
    ...
    function total(value, row, index){
        var totalnya = parseFloat(row.jumlah_barang)*parseFloat(row.harga_pembelian);
        return totalnya;
    }

    
    function actionsaldo(value, row, index){
		saldo_asli =  parseFloat(saldo_asli) + (parseFloat(row.jumlah_barang) * parseFloat(row.harga_pembelian));
    	return saldo_asli;
    }
    ...
    $pembayaran_hutang_table.bootstrapTable('refresh', {
        url: 'Laporan/get_return_pembelian_detail_tampil/'+no_faktur
    });
    ...
    function cekstatuskebijakankredit(value, row, index){
     ...
     //   console.log('penjualan/print_rugi_laba_salesman/'+tgl_1+'/'+tgl_2);
        $('#area-1').load(linke+'Laporan/print_return_pembelian/'+tgl_1+'/'+tgl_2, function() {
          printDiv('area-1');
